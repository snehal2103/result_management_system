...
class Register:
    def __init__(self, root ): # default constructor and root is a tkinter class object
        ...
        self.root.geometry("1600x780+0+0") # Setting width
        self.root.config(bg="white")
        self.bg=ImageTk.PhotoImage(file="image/background.jpg")
        bg=Label(self.root,image=self.bg).place(x=250,y=0,relwidth=1,relheight=1)
        self.left=ImageTk.PhotoImage(file="image/Epg.png")
        ...
        self.lbl_password_strength = Label(frame1, text="", font=("times new roman", 12), bg="white", fg="green")
        self.lbl_password_strength.place(x=50, y=380, width=400)
        ...
        btn_register=Button(self.root,text="Register Now",font=("goudy old style",15,"bold"),bg="green",fg="white",cursor="hand2",command=self.register_data).place(x=520,y=510)
        
        btn_login=Button(self.root,text="Sign In",font=("times new roman",20),bd=0,cursor="hand2",command=self.login_window).place(x=200,y=460,width=180)
    ...
    def validate_password_strength(self):
        ...
        if len(password) < 8:
            self.lbl_password_strength.config(text="Weak: Password should be at least 8 characters", fg="red")
        elif not re.search("[a-z]", password) or not re.search("[A-Z]", password) or not re.search("[0-9]", password):
            self.lbl_password_strength.config(text="Moderate: Password should contain lowercase, uppercase, and digits", fg="orange")
        else:
            self.lbl_password_strength.config(text="Strong: Password is secure", fg="green")
    ...
    def register_data(self):
        if not self.validate_contact_number():
            messagebox.showerror("Error", "Invalid Contact Number. Please enter a 10-digit number.", parent=self.root)
        elif not self.validate_email_format(self.txt_email.get()):
            messagebox.showerror("Error", "Invalid Email Address", parent=self.root)
        else:
            self.validate_password_strength()  
            
        if not self.validate_email_format(self.txt_email.get()):
            messagebox.showerror("Error", "Invalid Email Address", parent=self.root)
        else:
            self.validate_password_strength()  # Validate password strength
