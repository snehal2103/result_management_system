...
class resultClass:
    def __init__(self,root):
        ...
        txt_name=Entry(self.root,textvariable=self.var_name,font=("goudy old style",20,'bold'),bg='lightblue',state='readonly').place(x=280,y=160,width=300)
        txt_course=Entry(self.root,textvariable=self.var_course,font=("goudy old style",20,'bold'),bg='lightblue',state='readonly').place(x=280,y=220,width=300)
        txt_marks=Entry(self.root,textvariable=self.var_marks,font=("goudy old style",20,'bold'),bg='lightblue').place(x=280,y=280,width=300)
        txt_full_marks=Entry(self.root,textvariable=self.var_full_marks,font=("goudy old style",20,'bold'),bg='lightblue').place(x=280,y=340,width=300)
        
        btn_update = Button(self.root, text="Update", font=("times new roman", 15), bg="orange", activebackground="orange",
                    cursor="hand2", command=self.update)
        btn_update.place(x=560, y=420, width=120, height=35)
        ...
        self.btn_clear=Button(self.root,text="Clear",font=("times new roman",15),bg="lightgray",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)
        self.bg_img=Image.open("image/OIP.jpeg")
    ...
    def update(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                row = cur.fetchone()
                if row != None:
                # If result exists, update the marks
                  per = (int(self.var_marks.get()) * 100) / int(self.var_full_marks.get())
                  cur.execute("update result set marks_ob=?, full_marks=?, per=? where roll=? and course=?", (
                    self.var_marks.get(),
                    self.var_full_marks.get(),
                    str(per),
                    self.var_roll.get(),
                  ...
                  messagebox.showerror("Error", "No result found for this student and course", parent=self.root)
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to {str(ex)}")
